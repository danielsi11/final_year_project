...
for i_channel = 1:N_channel
    ...
    for i_alpha = 1:length(alphas)+1
        ...
        for i_PAC = 1:length(PACs)   
            ...
            %Overall Effective Channel and Noise Power
            for i_user = 1:K
                for i_gateway = 1:L
                    temp = zeros(1,B);
                    for i_cluster = 1:L
                        temp = temp + H(:,i_user,i_cluster)'*G(:,:,i_cluster,i_gateway);
                    end
                ...
                P_n_eff(i_user) = 1;
                for i_cluster = 1:L
                    P_n_eff(i_user) = P_n_eff(i_user) + norm(H(:,i_user,i_cluster))^2;
                end    
            ...
            if i_alpha == length(alphas)+1 %Perfect CSIT
                MMF_RS(i_PAC) = RS_perfect_rate(Gm,H_eff,PAC,tolerance,P_n_eff,G);
                MMF_NoRS(i_PAC) = NoRS_perfect_rate(Gm,H_eff,PAC,tolerance,P_n_eff,G);               
                
            else %Imperfect CSIT
                MMF_RS(i_PAC) = RS_imperfect_rate(Gm,H_est,PAC,tolerance,S,P_e,P_n_eff,G);
                MMF_NoRS(i_PAC) = NoRS_imperfect_rate(Gm,H_est,PAC,tolerance,S,P_e,P_n_eff,G); 
            end 
