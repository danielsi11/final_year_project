...
for i_cluster = 1:L
    ...
    for i_user = 1:sum(Gm(:,i_cluster))
        H_Cm = horzcat(H_Cm,H(:,i_user+offset,i_cluster));
    end
    [U_Cm,~,~] = svd(H_Cm);
...
while (loop)
    ...
    for i_user = 1:K         
        ...
        for i_cluster = 1:L
            if i_cluster ~= cluster_idx
                T_k(i_user) = T_k(i_user) + abs(H(:,i_user,i_cluster)'*p_c(:,:,i_cluster))^2;
            end    
        end    
    end
    ...
    for i_user = 1:K
        ...
        T_ck(i_user) = abs(H(:,i_user,cluster_idx)'*p_c(:,:,cluster_idx))^2 + T_k(i_user);
    end
    ...
    for i_user = 1:K
        ...
        g_ck(i_user) = p_c(:,:,i_cluster)'*H(:,i_user,i_cluster)*inv(T_ck(i_user));
    end
    ...
    for i_user = 1:K
        ...
        E_ck(i_user) = inv(T_ck(i_user))*(T_ck(i_user)-abs(H(:,i_user,i_cluster)'*p_c(:,:,i_cluster))^2);
    end
